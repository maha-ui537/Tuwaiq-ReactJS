...
const convertCurrency = () => {
    ...
    fetch(`https://api.currencylayer.com/convert?access_key=d115b2509425f1eae630720805e4d44b&from=${fromCurrency}&to=${toCurrency}&amount=${amount}`)
  ...
  <div className="container">
    <div className="row">
        ...
        <div className="card" style={{display:'flex',flexWrap:'wrap', backgroundColor:'#f9d1ac'}}>
            <div className="card-body">
                <h5 className="card-title">Currency Converter</h5>
                <p className="card-text">Convert currencies with real-time exchange rates.</p>
<div className="row mt-5">


    <div className="col">

        <select className="form-select" onChange={(p)=>setFromCurrency(p.target.value)}>
            <option value="">Select From Currency</option>
            {
                currancy.map((c) => (
                  <option key={c.id} value={c.Currency_code}>{c.country_name}</option>
    ))
            }
        </select>
    </div>
    ...
    <div className="col">
        <select className="form-select" onChange={(p)=>setToCurrency(p.target.value)}>
...
<div className="row mt-5">
    <div className="col">
        ...
        <input type="number" className="form-control" onChange={(p)=>setAmount(p.target.value)} />
    </div>
</div>
...
<div className="row mt-5">
    <div className="col">
        <button className="btn btn-primary" onClick={convertCurrency}>Convert</button>
    </div>
</div>
...
<div className="row mt-5">
    <div className="col">
        <h4 className=" badge bg-danger h-100 text-black">{result?.result?.toFixed(2)} {toCurrency}</h4>
    </div>
</div>
